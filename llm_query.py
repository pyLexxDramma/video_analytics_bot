...
- "набрали больше X просмотров по итоговой статистике" = views_count > X из таблицы videos, БЕЗ фильтров по дате!
...
- "по итоговой статистике" = таблица videos, БЕЗ фильтров по дате, БЕЗ использования video_snapshots!
"""
...
- "Сколько видео" = COUNT(*) из таблицы videos, а НЕ SUM()!
...
- "по итоговой статистике" = таблица VIDEOS, БЕЗ фильтров по дате!
...
- Если в запросе НЕ упоминается дата, НЕ добавляй фильтры по дате!
...
- Для фильтрации по диапазону дат используй DATE(created_at) BETWEEN 'YYYY-MM-DD' AND 'YYYY-MM-DD'
- ВАЖНО: Если в запросе указан конкретный ID креатора (например, "aca1061a9d324ecf8c3fa2bb32d7be63"), используй его ТОЧНО как есть, а НЕ заменяй на 'id'!
- Извлекай ID креатора из запроса и используй его в WHERE creator_id = 'реальный_id_из_запроса'
